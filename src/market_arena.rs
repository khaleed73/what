...
impl MarketArena {
    ...
    /// Returns a safe index (0) on out-of-bounds instead of panicking.
    #[inline(always)]
    pub fn get_index(&self, exchange_id: usize, token_id: usize) -> usize {
        ...
        if idx >= self.matrix.len() {
            tracing::error!(exchange_id, token_id, matrix_len = self.matrix.len(),
                "market_arena index out of bounds — returning 0");
            return 0;
        }
        idx
    }
