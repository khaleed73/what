...
///
/// Tracks starvation on a per-(exchange, token) basis using a `HashSet` so that
/// a single exchange recovering for one token does not mask starvation on other
/// exchanges or tokens.
pub struct CapitalStarvationDetector {
    /// Minimum balance threshold below which starvation is declared.
    min_threshold: Decimal,
    /// Set of (exchange_id, token_id) pairs currently in a starved state.
    starved_entries: std::sync::Mutex<std::collections::HashSet<(usize, usize)>>,
...
impl CapitalStarvationDetector {
    ...
    pub fn new(min_threshold: Decimal) -> Self {
        Self {
            min_threshold,
            starved_entries: std::sync::Mutex::new(std::collections::HashSet::new()),
    ...
    pub fn check_balance(
        ...
        if current_balance <= self.min_threshold {
            ...
            // H-5: Track per-(exchange, token) starvation state.
            let was_starved = {
                let mut starved_guard = self.starved_entries.lock().unwrap_or_else(|e| e.into_inner());
                let key = (exchange_id, token_id);
                let already = starved_guard.contains(&key);
                starved_guard.insert(key);
                already
            };
    ...
    pub fn is_starved(&self) -> bool {
        let guard = self.starved_entries.lock().unwrap_or_else(|e| e.into_inner());
        !guard.is_empty()
    }

    /// Clears starvation for a specific exchange+token pair, but only if the
    /// verified balance exceeds the minimum threshold (M-9). Returns `true` if
    /// the entry was successfully cleared, `false` if the balance is still
    /// insufficient.
    pub fn clear_starvation(&self, exchange_id: usize, token_id: usize, verified_balance: Decimal) -> bool {
        if verified_balance <= self.min_threshold {
            return false;
        }
        let mut guard = self.starved_entries.lock().unwrap_or_else(|e| e.into_inner());
        guard.remove(&(exchange_id, token_id));
        if guard.is_empty() {
            *self.last_event.lock().unwrap_or_else(|e| e.into_inner()) = None;
            tracing::info!(exchange_id, token_id, "Capital starvation cleared");
        } else {
            tracing::info!(
                exchange_id, token_id,
                remaining = guard.len(),
                "Capital starvation cleared for one entry ({} still starved)",
                guard.len()
            );
        }
        true
...
mod tests {
    ...
    fn test_clear_starvation() {
        ...
        assert!(det.is_starved());
        det.clear_starvation(0, 0, dec!(100.0));
        assert!(!det.is_starved());
    }
